...
<body>
	...
	<div class="centrar">
		...
		<button class="btn btn-outline-success"
			onclick="window.location.href = 'inserir.php'">Inserir Nou Vídeo</button>
	</div>
            ...
            echo "<div class='col'>";
                ...
                foreach ($data as $key => $value) {
                    if ($value != null) {
                        ...
                        if ($data[$key]["ambit"] == 0) {
                            echo "<button type='button' class = 'btn btn-outline-warning' onclick='ambit(1,$id)'> Pacient</button> \n";
                        } else if ($data[$key]["ambit"] == 1) {
                            echo "<button type='button' class = 'btn btn-outline-warning' onclick='ambit(0,$id)'> Professional</button> \n";
                        }
	...
	<script>
      	function amagar(amagar,id){
      		if(amagar == 0){
      			firebase.database().ref('LlistatVideosEs/'+id).update({'mostrar': 0});
						firebase.database().ref('LlistatVideosEn/'+id).update({'mostrar': 0});
      			firebase.database().ref('LlistatVideosCa/'+id).update({'mostrar': 0})
      			.then(result => window.location.reload(result));
      		}
      		else if(amagar == 1){
      			firebase.database().ref('LlistatVideosEs/'+id).update({'mostrar': 1});
      			firebase.database().ref('LlistatVideosEn/'+id).update({'mostrar': 1});
      			firebase.database().ref('LlistatVideosCa/'+id).update({'mostrar': 1})
      			.then(result => window.location.reload(result));
      		}
      	}
  	</script>
  		<script>
      	function ambit(ambit,id){
      		if(ambit == 0){
      			firebase.database().ref('LlistatVideosEs/'+id).update({'ambit': 0});
      			firebase.database().ref('LlistatVideosEn/'+id).update({'ambit': 0});
      			firebase.database().ref('LlistatVideosCa/'+id).update({'ambit': 0})
      			.then(result => window.location.reload(result));
      		}
      		else if(ambit == 1){
      			firebase.database().ref('LlistatVideosEs/'+id).update({'ambit': 1});
      			firebase.database().ref('LlistatVideosEn/'+id).update({'ambit': 1});
      			firebase.database().ref('LlistatVideosCa/'+id).update({'ambit': 1})
      			.then(result => window.location.reload(result));
      		}
      	}
  	</script>
